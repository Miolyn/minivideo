...
(
    ...
    `video_time`       int          NOT NULL COMMENT '在视频中发弹幕的时间',
    `color`            varchar(50)  NOT NULL COMMENT '弹幕颜色',
    `size`             int          NOT NULL COMMENT '弹幕大小',
    `position`         int          NOT NULL COMMENT '弹幕位置',
...
(
    ...
    `introduction`   varchar(255) not NULL COMMENT '社区简介',
    `avatar`         varchar(255) not NULL COMMENT '社区封面',
    `user_id`        varchar(50)  not NULL COMMENT '创建人id',
    `member_num`     int      default 0 COMMENT '成员数量',
    `activity_num`   int      default 0 COMMENT '帖子数量',
    `main_label_id`  int          not NULL COMMENT '主标签',
    `self_label_ids` varchar(255) NULL COMMENT '自定义标签',
    `created_at`     datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`     datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_deleted`     integer  default 0,
...
(
    ...
    `topic_name` varchar(255) NULL COMMENT '话题名称',
    `user_id`    varchar(50)  NULL COMMENT '第一个创建话题的人',
    `created_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_deleted` integer  default 0,
...
(
    ...
    `bullet_screen_num` int          NULL DEFAULT 0 COMMENT '弹幕数量',
    `created_at`        datetime          DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`        datetime          DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_deleted`        integer           default 0,
